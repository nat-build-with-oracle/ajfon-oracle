...
() => {
  ...
  function bgOf(el){ let e=el; while(e){ const b=getComputedStyle(e).backgroundColor;
    if(b && b!=='rgba(0, 0, 0, 0)' && b!=='transparent'){ const c=parse(b); if(c) return c; } e=e.parentElement; } return [255,255,255]; }
  const out=[];
...
pages = sorted(p.split('/')[-1] for p in glob.glob('html/ch*.html')) + ['index.html']
report = {}
